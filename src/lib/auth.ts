...
import { isLocalStoreMode, localGetUserById } from "./local-store";
...
function getSecret() {
  ...
  if (process.env.NODE_ENV !== "production") {
    return new TextEncoder().encode("dev-only-learning-session-secret");
  }
...
export function toSessionUser(
  user: Pick<DbUser, "id" | "username" | "role" | "display_name">,
): SessionUser {
  return {
    id: user.id,
    username: user.username,
